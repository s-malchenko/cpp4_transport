...
void RequestParserTest_SettingsJson()
{
    JsonParser parser;
    string str = R"({
  "base_requests": [
  ...
  "stat_requests": [
    ...
    auto request = parser.GetSettings(str);
    ASSERT(!request);
  ...
  "stat_requests": [
    ...
    request = parser.GetSettings(str);
    ASSERT((bool)request);
    auto settings = static_cast<SettingsRequest *>(request.value().get());
